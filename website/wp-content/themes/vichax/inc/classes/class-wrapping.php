 ...
 */
add_filter( 'template_include', array( 'Vichax_Wrapping', 'wrap' ), 99 );
add_filter( 'vichax_wrap_base', 'vichax_wrap_base_cpts' );
 ...
 */
function vichax_template_path() {
	return Vichax_Wrapping::$main_template;
}
 ...
 */
function vichax_template_base() {
	return Vichax_Wrapping::$base;
}

if ( ! class_exists( 'Vichax_Wrapping' ) ) {
	...
	class Vichax_Wrapping {
		...
		/**
		 * Initialize new instance of Vichax_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'vichax_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Vichax_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function vichax_wrap_base_cpts( $templates ) {
