...
if ( ! class_exists( 'Vichax_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Vichax_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'vichax' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'vichax' );
			$this->widget_id          = 'vichax-widget-taxonomy-tiles';
			...
			$this->settings           = array(
					...
					'category'=> array(
							...
							'label'  => esc_html__( 'Select category to show', 'vichax' ),
					...
					'description_length' => array(
							...
							'min_value'  => '0',
							'step_value' => '1',
							'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'vichax' ),
					),
					'show_post_count' => array(
							...
							'options' => array(
									'show_post_count_check' => esc_html__( 'Show post count', 'vichax' ),
							),
					),
					'layout_type' => array(
							'type'    => 'radio',
							...
							'options' => array(
									'grid' => array(
											'label' => esc_html__( 'Grid', 'vichax' ),
											'slave' => 'layout_type_grid',
									),
									'tiles' => array(
											'label' => esc_html__( 'Tiles', 'vichax' ),
									),
							),
					...
					'columns_number' => array(
							...
							'min_value'  => '1',
							'step_value' => '1',
							'label'      => esc_html__( 'Columns number ( layout type grid )', 'vichax' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				echo apply_filters( 'vichax_taxonomy_tiles_widget_before', '<div class="row grid ' . $layout_type . '-columns columns-number-' . $columns_number . '">', $instance );
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
							...
							'sufix'   => _n_noop( '%s post', '%s posts', 'vichax' ),
				...
				echo apply_filters( 'vichax_taxonomy_tiles_widget_after', '</div>', $instance );
			}
...
add_action( 'widgets_init', 'vichax_register_taxonomy_tiles_widget' );
function vichax_register_taxonomy_tiles_widget() {
	register_widget( 'Vichax_Taxonomy_Tiles_Widget' );
}
