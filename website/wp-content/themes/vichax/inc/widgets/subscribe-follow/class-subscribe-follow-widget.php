...
class Vichax_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'vichax' );
		$this->widget_id          = 'vichax_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'vichax' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'vichax' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'vichax' ),
				'label' => esc_html__( 'Subscribe Title', 'vichax' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'vichax' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'vichax' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'vichax' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'vichax' ),
				'label' => esc_html__( 'Subscribe submit label', 'vichax' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'vichax' ),
				'label' => esc_html__( 'Subscribe success', 'vichax' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'vichax' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'vichax' ),
				'label' => esc_html__( 'Follow Title', 'vichax' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'vichax' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'vichax' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'vichax' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'vichax' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'vichax' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'vichax' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'vichax' ),
					'top-center'    => esc_html__( 'Top Center', 'vichax' ),
					'top-right'     => esc_html__( 'Top Right', 'vichax' ),
					'center-left'   => esc_html__( 'Middle Left', 'vichax' ),
					'center'        => esc_html__( 'Middle Center', 'vichax' ),
					'center-right'  => esc_html__( 'Middle Right', 'vichax' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'vichax' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'vichax' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'vichax' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'vichax' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'vichax' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'vichax' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'vichax' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'vichax' ),
					'contain' => esc_html__( 'Contain', 'vichax' ),
					'auto'    => esc_html__( 'Auto', 'vichax' ),
				),
				'master' => 'background_image',
			),
		...
		add_action( 'wp_ajax_vichax_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_vichax_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'vichax' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'vichax' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'vichax' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'vichax' ),
		);

		$this->nonce = wp_create_nonce( 'vichax-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = locate_template( apply_filters( 'vichax_subscribe_widget_follow_template_file', 'inc/widgets/subscribe-follow/view/follow-view.php' ), false, false );
		$subscribe_template         = locate_template( apply_filters( 'vichax_subscribe_widget_subscribe_template_file', 'inc/widgets/subscribe-follow/view/subcribe-view.php' ), false, false );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'vichax' );
		}
	...
	public function get_social_nav() {
		return vichax_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = vichax_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'vichax-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'vichax_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function vichax_register_subscribe_follow_widgets() {
	register_widget( 'Vichax_Subscribe_Follow_Widget' );
}
