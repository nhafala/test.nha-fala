 ...
 * @package Vichax
 */
if ( ! class_exists( 'Vichax_About_Author_Widget' ) ) {

	/**
	 * Class Vichax_About_Author_Widget.
	 */
	class Vichax_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'vichax widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'vichax' );
			$this->widget_id          = 'vichax_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'vichax' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'vichax' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'vichax' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'vichax' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'vichax' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'vichax' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'vichax' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'vichax' ),
					'label' => esc_html__( 'Link label', 'vichax' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'vichax' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'vichax-author-avatar' );
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'vichax_widget_about_author_view', 'inc/widgets/about-author/view/about-author.php' ), false, false );
...
add_action( 'widgets_init', 'vichax_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function vichax_register_about_author_widgets() {
	register_widget( 'Vichax_About_Author_Widget' );
}
