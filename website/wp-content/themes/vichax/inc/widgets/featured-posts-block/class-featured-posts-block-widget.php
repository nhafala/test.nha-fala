...
if ( ! class_exists( 'Vichax_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Vichax_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'vichax' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'vichax' );
			$this->widget_id          = 'vichax_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => VICHAX_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'vichax' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'vichax' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'vichax' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'vichax' ),
						'categories' => esc_html__( 'Show categories', 'vichax' ),
						'tags'       => esc_html__( 'Show tags', 'vichax' ),
						'author'     => esc_html__( 'Show author', 'vichax' ),
						'date'       => esc_html__( 'Show date', 'vichax' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'vichax' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'vichax' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = locate_template(  apply_filters( 'vichax_featured_posts_block_widget_view_dir', 'inc/widgets/featured-posts-block/views/item.php' ), false, false );
			 ...
			 */
			$query = apply_filters( 'vichax_featured_posts_block_query', $query, $this->instance );
	...
	add_action( 'widgets_init', 'vichax_register_featured_posts_block_widget' );

	if ( false === function_exists( 'vichax_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function vichax_register_featured_posts_block_widget() {
			register_widget( 'Vichax_Featured_Posts_Block_Widget' );
		}
	}
}
