...
if ( ! class_exists( 'Vichax_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Vichax_Playlist_Slider_Widget.
	 */
	class Vichax_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'vichax' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'vichax' );
			$this->widget_id          = apply_filters( 'vichax_playlist_slider_widget_ID', 'vichax_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'vichax_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = vichax_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'vichax' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'vichax' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'vichax' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '500',
					'label' => esc_html__( 'Slider height ( px, rem )', 'vichax' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'vichax' ),
						true  => esc_html__( 'Buttons', 'vichax' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'vichax' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'vichax' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'vichax' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'vichax' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'vichax' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'vichax' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'vichax' ),
						'author'   => esc_html__( 'Author', 'vichax' ),
						'comments' => esc_html__( 'Comment count', 'vichax' ),
						'category' => esc_html__( 'Category', 'vichax' ),
						'post_tag' => esc_html__( 'Tag', 'vichax' ),
					),
					'label' => esc_html__( 'Display post meta data', 'vichax' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'vichax' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'vichax_playlist_slider_size', array(
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'vichax_playlist_buttons', false ),
...
add_action( 'widgets_init', 'vichax_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function vichax_register_playlist_slider_widget() {
	register_widget( 'Vichax_Playlist_Slider_Widget' );
}
