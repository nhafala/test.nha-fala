 ...
 * @return array
 */
function vichax_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! vichax_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'vichax_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'vichax_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'vichax' ) . $network['name'],
 ...
 * @return string
 */
function vichax_get_single_share_buttons() {
	ob_start();
	vichax_share_buttons( 'single' );
